...
def train(jf_game_id, train_start_date, train_end_date, val_start_date, val_end_date, save_path, features, model='DNN', device='cuda:0', max_row=None, minus_label_col=None):
    ...
    # model
    model = BaseModel(features=features, device=device, cross_net=model)
...
if __name__ == '__main__':
    ...
    for jf_game_id in ['mg']:
        for method in ['DNN', 'MoE', 'DCN']:
            save_path = os.path.join(get_root_path(), f'checkpoints/{jf_game_id}', f'{jf_game_id}_{method}_active_seed_42_test')
            train(jf_game_id, 805, 825, 826, 829,
                  save_path=save_path, features=ACTIVE_FEATURES, minus_label_col=None, model=method, max_row=None)
