...
from modeling.msdst_model import MSDSTModel
from modeling.perCLTV import PERModel
...
def train(jf_game_id, train_start_date, train_end_date, val_start_date, val_end_date, save_path, features, model='DNN', device='cuda:0', max_row=None, minus_label_col=None):
    ...
    # model
    model = MSDSTModel(features=features, device=device)
...
if __name__ == '__main__':
    ...
    for jf_game_id in ['mdnf']:
        save_path = os.path.join(get_root_path(), f'checkpoints/{jf_game_id}', f'{jf_game_id}_MSDST_6h_tc_seed_42_test')
    ...
    for jf_game_id in ['dhp']:
        save_path = os.path.join(get_root_path(), f'checkpoints/{jf_game_id}', f'{jf_game_id}_MSDST_6h_tc_seed_42_test')
        train(jf_game_id, 801, 807, 808, 808,
              save_path=save_path, features=ALL_FEATURES, minus_label_col='ltv_6h', max_row=None)
    # for jf_game_id in ['mdnf']:
    #     for method in ['DCN']:
