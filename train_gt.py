...
from modeling.gt_model import GTModel
...
def train(jf_game_id, train_start_date, train_end_date, val_start_date, val_end_date, save_path, features, device='cuda:0', max_row=None, minus_label_col=None):
    ...
    # model
    model = GTModel(features=features, device=device)
...
if __name__ == '__main__':
    ...
    # for jf_game_id in ['wslg']:
    #     for method in ['DNN', 'MoE', 'DCN']:
    #         save_path = os.path.join(get_root_path(), 'checkpoints/train_model', f'{jf_game_id}_{method}_layer_6h_tc_seed_42')
    #         train(jf_game_id, 805, 818, 819, 820,
    #               save_path=save_path, features=ALL_FEATURES, minus_label_col='ltv_6h', model=method, max_row=None)
    for jf_game_id in ['mdnf']:
        save_path = os.path.join(get_root_path(), 'checkpoints/train_model', f'{jf_game_id}_gt_active_th_seed_42')
        train(jf_game_id, 812, 825, 826, 827,
            save_path=save_path, features=ACTIVE_FEATURES, minus_label_col=None, max_row=None)
