...
class BaseModel(nn.Module):
    def __init__(self, features, sparse_embedding_dim=4,
        ...
        if cross_net == 'DNN':
            ...
            self.cross_layer = MoE(2, [self.hidden_dim, 3])
        elif cross_net == 'DCN':
            self.cross_layer = DCN(1, [self.hidden_dim, 8], self.hidden_dim, 8, 3)
        else:
    ...
    def forward(self, batch):
        ...
        ltv_delta = torch.stack([torch.zeros_like(labels['ltv_3h'], device=labels['ltv_3h'].device), labels['ltv_3h'],
                                 labels['ltv_6h'] - labels['ltv_3h'], labels['ltv3'] - labels['ltv_6h']], dim=-1).clamp(
            min=0.)
    ...
    def load_weights(self, path, strict):
